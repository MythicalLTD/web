...
The first step for installing Wings is to ensure we have the required directory structure setup. To do so, run the commands below, which will create the base directories and download the featherwings executable.
...
Running Wings in the background is a simple task, just make sure that it runs without errors before doing this. Place the contents below in a file called `featherwings.service` in the `/etc/systemd/system` directory.
