...
# Database logs
docker compose --project-directory "$MD_DIR" logs mythicaldash_v3_database
...
docker compose --project-directory "$MD_DIR" up -d mythicaldash_v3_database
# Wait for database to be ready, then:
docker exec -i mythicaldash_v3_database mysql -u root -p mythicaldash_remastered < /var/www/mythicaldash_backup.sql
