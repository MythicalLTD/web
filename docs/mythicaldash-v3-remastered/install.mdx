...
```bash
cd /var/www/mythicaldash-v3
make install # This will install frontend and backend dependencies but if you are looking for backend only dependencies (LITE VERSION) just run: cd backend && composer install
```
