...
```bash
make clean
make install # Or if you want to use only backend dependencies run: cd backend && composer install
```
...
```bash
make release
# Or in case if this fails you can use our prebuilt frontend
make get-frontend
```
